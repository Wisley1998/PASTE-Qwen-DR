...
|---:|---:|---:|:---:|---:|:---:|:---:|:---:|
| 64 | 32 | +0.6565 | [-0.2202, +1.5332] | +0.393% | [+0.287%, +0.500%] | regression | pass |
...
- Treatment uses a forked process sidecar, K=4 by default, finite leases, lazy result bridge, configured logical CPU placement, and SCHED_IDLE. All future batches are handed off in one bounded packet before the timed origin; `timed_parent_submit_packets=0` is enforced. No exact claims, result packets, terminal packets, or parent tombstone packets are permitted before the safety gate.
