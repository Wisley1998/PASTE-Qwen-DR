"""Online trace-learned tool/LLM closed-loop experiment primitives.

This module intentionally forks the frozen formal-v9 ``live_agent`` adapter.
New learned-prediction behavior lives here so historical formal code hashes and
their checked-in evidence remain unchanged.
...
class LiveClosedLoopExperiment:
    def __init__(
        ...
        *,
        ...
        self.visit_predictor = visit_predictor
        self._visit_result_tokens_ema = float(predicted_visit_result_tokens)
        self._ema_lock = asyncio.Lock()

    def _visit_prediction_record(
        self, candidates: Sequence[str], *, selected_url: str | None
    ) -> dict[str, Any]:
        predictor = self.visit_predictor
        artifact_sha256 = predictor.artifact_sha256 if predictor is not None else ""
        return {
            "policy": (
                predictor.policy
                if predictor is not None
                else "current-result-order-top-k"
            ),
            "artifact_sha256": artifact_sha256 or None,
            "top_k": predictor.top_k if predictor is not None else self.visit_top_k,
            "candidate_urls": list(candidates),
            "candidate_count": len(candidates),
            "selected_url_exact_hit": (
                selected_url in candidates if selected_url is not None else None
            ),
        }
    ...
    async def run_task(
        ...
        *,
        ...
        visit_prediction_candidates: list[str] = []
        ...
        try:
            ...
            if (
                ...
                if self.visit_predictor is None:
                    visit_prediction_candidates = visit_candidates[: self.visit_top_k]
                else:
                    allowed = set(visit_candidates)
                    visit_prediction_candidates = [
                        url
                        for url in self.visit_predictor.predict_structured_result(
                            search_result.result
                        )
                        if url in allowed
                    ]
                for rank, url in enumerate(visit_prediction_candidates, start=1):
                    await self.broker.speculate(
                        Invocation(
            ...
            return {
                ...
                "visit_prediction": self._visit_prediction_record(
                    visit_prediction_candidates, selected_url=selected_url
                ),
                "answer": answer,
            ...
            return {
                ...
                "visit_prediction": self._visit_prediction_record(
                    visit_prediction_candidates, selected_url=None
                ),
                "guided_json_recovery": _guided_json_recovery_summary(
                    guided_json_records
                ),
